...
public class DishControllerTest extends BaseControllerTest{
    ...
    @ExpectedDatabases({
        ...
        @ExpectedDatabase(value = "dish/InitialData.xml", table = DbTable.Phone.TABLE, override = false),
        @ExpectedDatabase(value = "dish/InitialData.xml", table = DbTable.Address.TABLE, override = false),
        @ExpectedDatabase(value = "dish/operations/DishAdd.xml", table = DbTable.Provider.TABLE, override = false),
    ...
    public void shouldAddNewDish() throws Exception {
        ...
        Dish[] addedDishes = {new Dish.Builder(EntityState.ADDED, "Added dish", ActivityStatus.ACTIVE).
                ...
                .dishComponents(Arrays.asList(
                        new DishComponent.Builder(EntityState.ADDED, "Added dish component 1", ActivityStatus.ACTIVE)
                            .maximumNumberOfElements(3).numberOfRequiredElements(1).useComponentPriceAsDishPrice(YesNo.NO)
                            .dishElements(Arrays.asList(
                                    new DishElement.Builder(EntityState.ADDED, ActivityStatus.ACTIVE)
                                    .name("Added element 1").description("Added element 1 desc").build(),
                                    new DishElement.Builder(EntityState.ADDED, ActivityStatus.ACTIVE)
                                    .name("Added element 2").description("Added element 2 desc").build()))
                            .build(),
                        new DishComponent.Builder(EntityState.ADDED, "Added dish component 2", ActivityStatus.ACTIVE)
                            .maximumNumberOfElements(2).numberOfRequiredElements(0).useComponentPriceAsDishPrice(YesNo.NO).build()))
                .build()};
        ...
        mockMvc.perform(MockMvcRequestBuilders.post(Endpoint.PROVIDER + "/1/" + Endpoint.DISH)
                ...
                .content(RestTestUtil.convertObjectToJsonBytes(addedDishes)))
                .andDo(MockMvcResultHandlers.print())
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.content().contentType(MediaType.APPLICATION_JSON))
    ...
    @ExpectedDatabases({
        ...
        @ExpectedDatabase(value = "dish/InitialData.xml", table = DbTable.Phone.TABLE, override = false),
        @ExpectedDatabase(value = "dish/InitialData.xml", table = DbTable.Address.TABLE, override = false),
        @ExpectedDatabase(value = "dish/operations/DishDelete.xml", table = DbTable.Provider.TABLE, override = false),
    ...
    public void shouldDeleteDishWithComponents() throws Exception {
        ...
        Dish toRemove = dishes[1];
        toRemove.getEntityAspect().setEntityState(EntityState.DELETED);

        mockMvc.perform(MockMvcRequestBuilders.post(Endpoint.PROVIDER + "/1/" + Endpoint.DISH)
                .contentType(RestTestUtil.APPLICATION_JSON_UTF8)
                .content(RestTestUtil.convertObjectToJsonBytes(new Dish[] {toRemove})))
                .andDo(MockMvcResultHandlers.print())
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.content().contentType(MediaType.APPLICATION_JSON))
                .andReturn().getResponse();
    ...
    @ExpectedDatabases({
        ...
        @ExpectedDatabase(value = "dish/InitialData.xml", table = DbTable.Phone.TABLE, override = false),
        @ExpectedDatabase(value = "dish/InitialData.xml", table = DbTable.Address.TABLE, override = false),
        @ExpectedDatabase(value = "dish/operations/DishDeleteElem.xml", table = DbTable.Provider.TABLE, override = false),
        @ExpectedDatabase(value = "dish/operations/DishDeleteElem.xml", table = DbTable.Dish.TABLE, override = false),
        @ExpectedDatabase(value = "dish/operations/DishDeleteElem.xml", table = DbTable.DishComponent.TABLE, override = false),
        @ExpectedDatabase(value = "dish/operations/DishDeleteElem.xml", table = DbTable.DishElement.TABLE, override = false),
    })
    ...
    @ExpectedDatabases({
        ...
        @ExpectedDatabase(value = "dish/InitialData.xml", table = DbTable.Phone.TABLE, override = false),
        @ExpectedDatabase(value = "dish/InitialData.xml", table = DbTable.Address.TABLE, override = false),
        @ExpectedDatabase(value = "dish/operations/ModifyDish.xml", table = DbTable.Provider.TABLE, override = false),
        @ExpectedDatabase(value = "dish/operations/ModifyDish.xml", table = DbTable.Dish.TABLE, override = false),
        @ExpectedDatabase(value = "dish/operations/ModifyDish.xml", table = DbTable.DishComponent.TABLE, override = false),
        @ExpectedDatabase(value = "dish/operations/ModifyDish.xml", table = DbTable.DishElement.TABLE, override = false),
    })
    public void shouldModifyDishWithComponentsAndElements() throws Exception {
        ...
        toModify.getEntityAspect().setEntityState(EntityState.MODIFIED);
        ...
        toModify.getDishComponents().get(0).getEntityAspect().setEntityState(EntityState.DELETED);
        toModify.getDishComponents().get(1).setDescription("Modified component description");
        toModify.getDishComponents().add(
                new DishComponent.Builder(EntityState.ADDED, "Added kebab component", ActivityStatus.ACTIVE)
                .maximumNumberOfElements(1).numberOfRequiredElements(1).useComponentPriceAsDishPrice(YesNo.NO).build());

        mockMvc.perform(MockMvcRequestBuilders.post(Endpoint.PROVIDER + "/1/" + Endpoint.DISH)
                .contentType(RestTestUtil.APPLICATION_JSON_UTF8)
                .content(RestTestUtil.convertObjectToJsonBytes(new Dish[] {toModify})))
                .andDo(MockMvcResultHandlers.print())
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.content().contentType(MediaType.APPLICATION_JSON))
                .andReturn().getResponse();
